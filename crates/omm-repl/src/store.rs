...
impl FeatureStore {
    ...
    pub fn open(path: &Path) -> OmmResult<Self> {
        let db = Database::create(path)
            .map_err(|e| OmmError::Other(format!("Failed to open feature store: {e}")))?;

        // Ensure table exists
        let txn = db
            .begin_write()
            .map_err(|e| OmmError::Other(format!("Failed to begin write: {e}")))?;
        {
            let _table = txn
                .open_table(BBOXES)
                .map_err(|e| OmmError::Other(format!("Failed to open table: {e}")))?;
        }
        txn.commit()
            .map_err(|e| OmmError::Other(format!("Failed to commit: {e}")))?;

        Ok(Self { db })
    }

    /// Store a feature's bounding box. Returns the old bbox if it existed.
    pub fn upsert(&self, id: i64, bbox: &BBox) -> OmmResult<Option<BBox>> {
        ...
        let txn = self
            .db
            .begin_write()
            .map_err(|e| OmmError::Other(format!("Write txn failed: {e}")))?;
        {
            let mut table = txn
                .open_table(BBOXES)
                .map_err(|e| OmmError::Other(format!("Open table failed: {e}")))?;
            table
                .insert(id, bytes.as_slice())
                .map_err(|e| OmmError::Other(format!("Insert failed: {e}")))?;
        }
        txn.commit()
            .map_err(|e| OmmError::Other(format!("Commit failed: {e}")))?;

        Ok(old)
    }

    /// Delete a feature's bbox by ID. Returns the old bbox if it existed.
    pub fn delete(&self, id: i64) -> OmmResult<Option<BBox>> {
        let old = self.get_bbox(id)?;
    ...
    pub fn get_bbox(&self, id: i64) -> OmmResult<Option<BBox>> {
        let txn = self
            .db
            .begin_read()
            .map_err(|e| OmmError::Other(format!("Read txn failed: {e}")))?;
        let table = txn
            .open_table(BBOXES)
            .map_err(|e| OmmError::Other(format!("Open table failed: {e}")))?;
        ...
        match table.get(id) {
            ...
            Ok(None) => Ok(None),
            Err(e) => Err(OmmError::Other(format!("Get failed: {e}"))),
        }
    }

    /// Count of features in the store.
    pub fn len(&self) -> OmmResult<u64> {
        let txn = self
            .db
            .begin_read()
            .map_err(|e| OmmError::Other(format!("Read txn failed: {e}")))?;
    ...
    pub fn is_empty(&self) -> OmmResult<bool> {
        self.len().map(|n| n == 0)
    }
}
