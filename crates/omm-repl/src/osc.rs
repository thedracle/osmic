...
pub fn parse_osc_gz(path: &Path) -> OmmResult<Vec<OscChange>> {
    let file = std::fs::File::open(path)
        .map_err(|e| OmmError::Other(format!("Failed to open {}: {e}", path.display())))?;
    let decoder = GzDecoder::new(file);
    parse_osc(decoder)
}

/// Parse an .osc.gz from raw bytes (e.g. downloaded from replication server).
pub fn parse_osc_gz_bytes(data: &[u8]) -> OmmResult<Vec<OscChange>> {
    let decoder = GzDecoder::new(data);
    parse_osc(decoder)
}

/// Parse OSC XML from any reader.
pub fn parse_osc<R: Read>(reader: R) -> OmmResult<Vec<OscChange>> {
    ...
    loop {
        ...
        match event {
            ...
            Ok(Event::Eof) => break,
            Err(e) => {
                return Err(OmmError::Other(format!("XML parse error: {e}")));
            }
            _ => {}
        }
        buf.clear();
