...
impl ExternalFeatureSort {
    ...
    fn flush_chunk(&mut self) -> io::Result<()> {
        ...
        let path = self
            .tmp_dir
            .join(format!("omm_sort_chunk_{:06}.bin", self.chunk_counter));
...
mod tests {
    ...
    fn external_sort_single_chunk() {
        let tmp = std::env::temp_dir().join("omm_sort_test_single");
    ...
    fn external_sort_multi_chunk() {
        let tmp = std::env::temp_dir().join("omm_sort_test_multi");
        let _ = fs::create_dir_all(&tmp);
    ...
    fn temp_files_cleaned_up() {
        let tmp = std::env::temp_dir().join("omm_sort_test_cleanup");
        let _ = fs::create_dir_all(&tmp);
        ...
        // No omm_sort_chunk files should remain
