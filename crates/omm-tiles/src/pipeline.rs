...
impl<'a> TileGenerator<'a> {
    ...
    fn generate_tiles_gpu<F>(
        &self,
        accel: &omm_accel::GpuAccelerator,
    ...
    where
        F: FnMut(TileCoord, &[u8]) -> OmmResult<()>,
    {
        use omm_accel::metal::flatten::WorkItem;
        use omm_accel::metal::accelerator::PendingBatch;
        ...
        for tile_batch in &batches {
            ...
            let mut simplified_geoms: Vec<omm_core::geometry::Geometry> = Vec::new();
            ...
            let new_pending = if !work_items.is_empty() {
                accel.clip_batch_async(&work_items).map_err(|e| {
                    omm_core::error::OmmError::Tile(format!("GPU dispatch failed: {e}"))
                })?
            } else {
                None
            };
    ...
    #[cfg(feature = "native")]
    pub fn generate_all_streaming<F>(&self, write_tile: &mut F) -> OmmResult<u64>
    where
        F: FnMut(TileCoord, &[u8]) -> OmmResult<()>,
    {
        use omm_core::error::OmmError;

        // Place temp files in the system temp directory.
        let tmp_dir = env::temp_dir().join("omm_tile_sort");
        std::fs::create_dir_all(&tmp_dir).map_err(|e| {
            OmmError::Tile(format!("Cannot create sort tmp dir {tmp_dir:?}: {e}"))
        })?;
        ...
        for z in self.config.min_zoom..=self.config.max_zoom {
                ...
                for ty in min_y..=max_y {
                    for tx in min_x..=max_x {
                        ...
                        sorter.add(key, idx).map_err(|e| {
                            OmmError::Tile(format!("External sort write failed: {e}"))
                        })?;
                        scatter_count += 1;
                    }
                }
            ...
            let sorted = sorter.finish().map_err(|e| {
                OmmError::Tile(format!("External sort finish failed: {e}"))
            })?;
...
fn gpu_output_to_geometry(
    ...
    count: u32,
    is_area: bool,
) -> Option<omm_core::geometry::Geometry> {
    ...
    if is_area && n >= 3 {
        Some(omm_core::geometry::Geometry::Polygon(Polygon::new(
            LineString::new(coords),
            vec![],
        )))
    } else {
        Some(omm_core::geometry::Geometry::Line(LineString::new(coords)))
    }
}
