...
pub fn write_csv(entities: &[Entity], path: &Path) -> OmmResult<()> {
    let file = std::fs::File::create(path).map_err(OmmError::Io)?;
    ...
    writer.flush().map_err(OmmError::Io)?;
    Ok(())
}
...
pub fn write_json(entities: &[Entity], path: &Path) -> OmmResult<()> {
    ...
    std::fs::write(path, json).map_err(OmmError::Io)
}
...
pub fn write_geojson(entities: &[Entity], path: &Path) -> OmmResult<()> {
